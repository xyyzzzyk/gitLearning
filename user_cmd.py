...
fh = logging.FileHandler('user_cmd.log')
...
def ssh_connect(host, pwd, puser):      #connect and lock user
    ...
    try:
        ...
        r_command =["sudo sed -i 's/KexAlgorithms /#KexAlgorithms/' /etc/ssh/sshd_config","sudo service sshd restart"] 
        
        for rcmd in r_command :
            stdin, stdout, stderr = s.exec_command(rcmd, get_pty=True)
    
   
            logger.info("[change_result] host: " + host.center(16) +
                            " ampmon/" + pwd.center(16) + rcmd+ "........output: " + stdout.read().decode())
    except Exception as e:
